...
| `apt.key`         | Fuegt einen GPG-Schluessel fuer APT hinzu. Laedt den Key nur via HTTPS, prueft den heruntergeladenen Schluessel gegen einen expliziten OpenPGP-Fingerprint und speichert ihn unter `/etc/apt/keyrings/<name>.gpg` (moderne Methode, kein `apt-key`). Wird von `apt.repository` via `signed-by` referenziert. | Pruefung ob `/etc/apt/keyrings/<name>.gpg` existiert und denselben Fingerprint hat | einfach |

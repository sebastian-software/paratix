...
describe("net.hosts — apply", () => {
  ...
  it("aborts the write when /etc/hosts changes between read and write", async () => {
    ...
    mockSsh.readFile = async (remotePath: string): Promise<string> => {
      readCount += 1
      // oxlint-disable-next-line vitest/no-conditional-in-test -- simulating the race between an initial read and a concurrent /etc/hosts modification before the guarded re-read requires a counter-based switch in a single mock function

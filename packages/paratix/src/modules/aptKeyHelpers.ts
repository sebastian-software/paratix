...
import { allocateGpgHomedir, dearmorAptKeyToKeyring } from "./aptKeyStaging.js"
import {
...
/**
 * Parse the `gpg --show-keys` colon output, enforcing exactly one primary key.
 *
 * @param parameters - Inspection inputs.
 * @param parameters.name - Logical apt.key name used in error messages.
 * @param parameters.stdout - Raw colon-separated output from `gpg --show-keys`.
 * @returns The single primary key fingerprint, or a failed ModuleResult on
 *   parse errors or unexpected multi-key material.
 */
function extractSinglePrimaryFingerprint(parameters: { name: string; stdout: string }): {
  fingerprint: string
  result: ModuleResult
} {
  const fingerprints = parseOpenPgpPrimaryKeyFingerprints(parameters.stdout)
  ...
  if (fingerprints.length > 1) {
    return {
      fingerprint: "",
      result: failed(
        `[apt.key] key material for ${parameters.name} contains ${fingerprints.length} primary keys`
      ),
    }
  }
...
async function inspectOpenPgpFingerprint(
  ...
  try {
    ...
    return extractSinglePrimaryFingerprint({ name, stdout: fingerprintResult.stdout })
  } finally {

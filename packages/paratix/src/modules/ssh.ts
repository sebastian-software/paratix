...
async function replaceKnownHostsAtomically(
  ...
  const backupPath = `${quotedKnownHostsPath}.paratix-backup.$$`
  const replaceResult = await conn.exec(
    `chmod 600 ${quotedTemporaryPath} && { expected_known_hosts_hash=$(sha256sum ${quotedTemporaryPath} | cut -d' ' -f1) || exit $?; [ ! -L ${quotedSshDirectoryPath} ] || { echo '.ssh must not be a symlink' >&2; exit 1; }; [ -d ${quotedSshDirectoryPath} ] || { echo '.ssh must be a directory' >&2; exit 1; }; [ ! -L ${quotedKnownHostsPath} ] || { echo 'known_hosts must not be a symlink' >&2; exit 1; }; known_hosts_backup=; rollback_known_hosts() { if [ -n "$known_hosts_backup" ] && [ -e "$known_hosts_backup" ]; then mv -T -- "$known_hosts_backup" ${quotedKnownHostsPath} || { echo 'known_hosts rollback failed after replace error' >&2; return 1; }; fi; }; if [ -e ${quotedKnownHostsPath} ]; then [ -f ${quotedKnownHostsPath} ] || { echo 'known_hosts must be a regular file' >&2; exit 1; }; known_hosts_backup=${backupPath}; [ ! -e "$known_hosts_backup" ] || { echo 'known_hosts backup path already exists' >&2; exit 1; }; mv -T -- ${quotedKnownHostsPath} "$known_hosts_backup" || exit $?; fi; [ ! -e ${quotedKnownHostsPath} ] || { echo 'known_hosts was recreated during replace; refusing to clobber' >&2; rollback_known_hosts; exit 1; }; mv -T -- ${quotedTemporaryPath} ${quotedKnownHostsPath} || { replace_status=$?; rollback_known_hosts || exit $?; exit "$replace_status"; }; if [ -n "$known_hosts_backup" ]; then rm -f -- "$known_hosts_backup" || exit $?; fi; [ ! -e ${quotedTemporaryPath} ] || { echo 'known_hosts replace did not consume temporary file' >&2; exit 1; }; [ ! -L ${quotedKnownHostsPath} ] || { echo 'known_hosts must not be a symlink' >&2; exit 1; }; [ -f ${quotedKnownHostsPath} ] || { echo 'known_hosts must be a regular file' >&2; exit 1; }; known_hosts_state=$(stat -c '%a %F' ${quotedKnownHostsPath}) || exit $?; [ "$known_hosts_state" = ${expectedKnownHostsState} ] || { echo 'known_hosts metadata changed during replace' >&2; exit 1; }; known_hosts_hash=$(sha256sum ${quotedKnownHostsPath} | cut -d' ' -f1) || exit $?; [ "$known_hosts_hash" = "$expected_known_hosts_hash" ] || { echo 'known_hosts content changed during replace' >&2; exit 1; }; }`,
    MUTATION_EXEC_OPTS
  )
...
// R-0000835: extract the check body so the surrounding `check` lambda
// only carries the try/catch shell. Keeping the lambda flat keeps the
// `complexity` lint budget headroom for future trust-anchor variants.
async function evaluateKnownHostsCheck(
  conn: SshConnection,
  parameters: { host: string; options?: KnownHostsOptions; state: KnownHostsState }
): Promise<"needs-apply" | "ok"> {
  const { host, options, state } = parameters
  ...
  knownHosts(host: string, options?: KnownHostsOptions): Module {
    ...
    return {
      ...
      async check(conn: null | SshConnection): Promise<"needs-apply" | "ok"> {
        ...
        try {
          return await evaluateKnownHostsCheck(conn, { host, options, state })
        } catch (error) {
